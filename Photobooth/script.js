...
function uploadImageToBannerAPI(file) {
  ...
  fetch("http://127.0.0.1:5000/process_image", {
    ...
    .then((data) => {
      ...
      // genQR(data.id);
      console.log('this is from bg remove api',data);
      let imageDataUrl = "data:image/jpg;base64," + data.image;
...
function uploadImageToStorageAPI(file) {
  ...
  // Make a POST request to the API
  fetch("https://api2-photobooth.xri.com.bd/upload", {
    method: "POST",
    headers: {
    ...
    .then((response) => {
      console.log('this is from stroage api',response);
      if (response.ok) {
        return response.json();
      } else {
        throw new Error("Failed to upload image");
    ...
    .then((data) => {
      console.log('inside 2nd promise',data);
      // Handle the API response here
      toggleSpinner(false);
